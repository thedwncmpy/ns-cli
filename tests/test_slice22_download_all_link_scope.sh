...
set -euo pipefail
args="$*"
printf "%s\n" "$args" >> "${SLICE22_CURL_LOG:?}"

if [[ "$args" == *"-X POST"*"/v1/databases/"*"/query"*'rel_123'* ]]; then
  printf '{"results":[{"id":"page_alpha","properties":{"Name":{"id":"title","type":"title","title":[{"plain_text":"alpha"}]},"relation_prop":{"id":"rel","type":"relation","relation":[{"id":"rel_123"}]}}},{"id":"page_beta","properties":{"Name":{"id":"title","type":"title","title":[{"plain_text":"beta"}]},"relation_prop":{"id":"rel","type":"relation","relation":[{"id":"rel_123"}]}}}],"has_more":false}'
...
export SLICE22_CURL_LOG="$tmp_dir/curl.log"
...
set +e
out="$(cd "$notes_root/project" && "$CLI" download-all 2>&1)"
code=$?
set -e
assert_exit_code "$code" 0
assert_contains "$out" "Downloaded 'alpha'"
assert_contains "$out" "Downloaded 'beta'"
assert_contains "$out" "Processed 2 page(s)."
...
[[ "$query_count" -eq 1 ]] || fail "expected 1 database query, got $query_count"

echo "PASS: slice 22 download-all link scope"
