...
set +e
snippets_out="$($CLI snippets ultisnips 2>&1)"
code=$?
set -e
assert_exit_code "$code" 0
assert_contains "$snippets_out" "priority -50"
assert_contains "$snippets_out" "snippet ns-toggle3"
assert_contains "$snippets_out" "snippet ns-todo"
